...
export default function App() {
  ...
  const handleFlip = (id) => {
    ...
    if (flippedId === id) {
      setActiveFlipId(id);
      Animated.timing(flipAnim, { toValue: 0, duration: 320, useNativeDriver: true }).start(() => {
        setFlippedId(null);
  ...
  const sendScintilla = (p) => {
    if (!sent.includes(p.id)) {
      ...
      setTimeout(() => {
        if (Math.random() > 0.25) {
          setMatches((prev) => (prev.includes(p.id) ? prev : [...prev, p.id]));
          triggerMatchPopup(p);
        }
      }, 800);
    }
  ...
  const handleSwipeRight = () => {
    ...
    Animated.timing(swipePosition, { toValue: { x: 550, y: -60 }, duration: 230, useNativeDriver: true }).start(() => {
      sendScintilla(topProfile);
      goNextCard();
    });
  };
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (_, g) => {
        ...
        swipePosition.setValue({ x: g.dx, y: g.dy * 0.25 });
      },
      onPanResponderRelease: (_, g) => {
        if (flippedId || showMatch) return;
        if (Math.abs(g.dx) < 8 && Math.abs(g.dy) < 8) {
          handleFlip(topProfile.id);
          Animated.spring(swipePosition, { toValue: { x: 0, y: 0 }, useNativeDriver: true }).start();
          return;
        }
        if (g.dx > 110) handleSwipeRight();
        else if (g.dx < -110) handleSwipeLeft();
        else Animated.spring(swipePosition, { toValue: { x: 0, y: 0 }, friction: 6, useNativeDriver: true }).start();
      },
    })
  ).current;
  ...
  const isFlipped = topProfile && flippedId === topProfile.id;
  const isFlipping = topProfile && activeFlipId === topProfile.id;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.counterPill}>
        ...
        <Text style={styles.counterTextPremium}>Inviate {sent.length} • Match {matches.length}</Text>
        <TouchableOpacity onPress={() => { setSent([]); setMatches([]); setSwipeIndex(0); }} style={styles.resetPill}><Text style={styles.resetTextPremium}>Reset</Text></TouchableOpacity>
      </View>
      ...
      {tab === 'scopri' && (
        <View style={styles.filterWrapperPremium}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterContentPremium}>
            {cities.map((c) => (
              <TouchableOpacity key={c} style={[styles.filterChipPremium, filterCity === c && styles.filterChipActivePremium]} onPress={() => { setFilterCity(c); setSwipeIndex(0); }}>
          ...
          <View style={styles.deckContainer}>
            {nextNextProfile && (
              <View style={[styles.premiumCard, styles.cardBehind2]}>
                ...
                <View style={styles.premiumOverlaySmallFIXED} />
              </View>
            )}
            {nextProfile && (
            ...
            {isFlipped || isFlipping ? (
                ...
                <Animated.View style={[styles.flipCard, styles.flipBackCard, { transform: [{ perspective: 1000 }, { rotateY: isFlipping ? backInterpolate : '360deg' }] }]}>
                  <View style={styles.cardBack}>
                    <View style={styles.cardBackHeader}>
                      ...
                      <View style={{ flex: 1, marginLeft: 12 }}><Text style={styles.cardBackName}>{topProfile.name}</Text><Text style={styles.cardBackScore}>{topProfile.score} ❤️ • {topProfile.city}</Text></View>
                      <View style={styles.closeFlip}><Text style={styles.closeFlipText}>✕</Text></View>
                    </View>
                    ...
                    {topProfile.reviews.map((r, i) => (
                      <View key={i} style={styles.reviewItem}><Text style={styles.reviewAuthor}>{r.author}</Text><Text style={styles.reviewText}>"{r.text}"</Text></View>
                    ))}
                    <View style={styles.backButtonFlip}><Text style={styles.backButtonFlipText}>↩️ Tocca ovunque per tornare alla foto</Text></View>
                  </View>
                </Animated.View>
              </Pressable>
            ) : (
              <Animated.View {...panResponder.panHandlers} style={[styles.premiumCard, styles.cardTop, { transform: [{ translateX: swipePosition.x }, { translateY: swipePosition.y }, { rotate: rotate }] }]}>
                <Image source={{ uri: topProfile.photo }} style={styles.premiumImage} />
                <View style={styles.premiumOverlaySmallFIXED} />
                <Animated.View style={[styles.likeBadge, { opacity: likeOpacity }]}><Text style={styles.likeText}>SCINTILLA ✨</Text></Animated.View>
                <Animated.View style={[styles.nopeBadge, { opacity: nopeOpacity }]}><Text style={styles.nopeText}>SCARTA ✕</Text></Animated.View>
                <View style={styles.scoreBadgePremium}><Text style={styles.scoreTextPremium}>{topProfile.score} ❤️</Text></View>
                <View style={styles.premiumCardContent}><Text style={styles.premiumName}>{topProfile.name}</Text><Text style={styles.premiumCity}>{topProfile.city} • {topProfile.distance} • {topProfile.bio}</Text></View>
              </Animated.View>
            )}

            <View style={styles.swipeButtonsRow}>
              <TouchableOpacity style={[styles.swipeBtn, styles.swipeBtnNo]} onPress={handleSwipeLeft}><Text style={styles.swipeBtnText}>✕</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.swipeBtn, styles.swipeBtnYes]} onPress={handleSwipeRight}><Text style={styles.swipeBtnTextYes}>✨</Text></TouchableOpacity>
            </View>
          </View>
        )}

        {tab === 'inviate' && inviateProfiles.length === 0 && (
          <View style={styles.emptyBoxPremium}><Text style={styles.emptyIcon}>⏳</Text><Text style={styles.emptyTextPremium}>Nessuna in attesa</Text></View>
        )}
        {tab === 'inviate' && inviateProfiles.map((p) => (
          <View key={p.id} style={styles.premiumCardSmall}><Image source={{ uri: p.photo }} style={styles.avatarPremium} /><View style={{ flex: 1, marginLeft: 12 }}><Text style={styles.namePremium}>{p.name}</Text><Text style={styles.cityPremium}>{p.city}</Text></View></View>
        ))}

        {tab === 'match' && matchProfiles.length === 0 && (
          <View style={styles.emptyBoxPremium}><Text style={styles.emptyIcon}>🔥</Text><Text style={styles.emptyTextPremium}>Nessun Match</Text></View>
        )}
      ...
      {showMatch && (
        <View style={styles.matchOverlay}>
          <Animated.View style={[styles.matchPopup, { transform: [{ scale: matchScale }] }]}>
            <Text style={styles.matchPopupTitle}>È un Match! ❤️‍🔥</Text>
            <Text style={styles.matchPopupSub}>Tu e {showMatch.name} vi siete piaciuti</Text>
            <View style={styles.matchAvatarsRow}>
              <Image source={{ uri: myProfileData.photo }} style={styles.matchPopupAvatar} />
              <Text style={styles.matchHeart}>❤️</Text>
              <Image source={{ uri: showMatch.photo }} style={styles.matchPopupAvatar} />
            </View>
            <TouchableOpacity style={styles.matchPopupPrimary} onPress={() => { setActiveChat(showMatch); setShowMatch(null); }}><Text style={styles.matchPopupPrimaryText}>Manda un messaggio 💬</Text></TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  swipeButtonsRow: { position: 'absolute', bottom: 0, flexDirection: 'row', gap: 30 },
